...
def spotfi_algorithm_1_package_one(csi_matrix):
    R = abs(csi_matrix)
    phase_matrix = np.vstack((np.unwrap(map(phase, csi_matrix[0, :])), np.unwrap(map(phase, csi_matrix[1, :])),
                              np.unwrap(map(phase, csi_matrix[2, :]))))
    fit_X = np.concatenate((np.linspace(1, 30, 30), np.linspace(1, 30, 30), np.linspace(1, 30, 30)))
    fit_Y = np.concatenate((np.unwrap(map(phase, csi_matrix[0, :])), np.unwrap(map(phase, csi_matrix[1, :])),
                            np.unwrap(map(phase, csi_matrix[2, :]))))
...
def aoa_tof_music(x, antenna_distance, frequency, sub_freq_delta, theta_range, tau_range):
    ...
    for ii in range(0, len(theta_range)):
        for jj in range(0, len(tau_range)):
            steering_vector = compute_steering_vector(theta_range[ii], tau_range[jj], frequency, sub_freq_delta,
                                                      antenna_distance)
